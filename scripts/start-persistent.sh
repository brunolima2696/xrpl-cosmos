...
if [ ! -f "$HOMEDIR/config/genesis.json" ]; then
  echo "$CHAIN_NAME ainda não inicializada. Criando genesis com chain-id $CHAINID..."
...
echo "$CHAIN_NAME já inicializada. Subindo estado existente com chain-id $CHAINID..."
